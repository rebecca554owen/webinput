...
async function initApp() {
    ...
    if (!invoke) {
        console.log('Tauri API 尚未就绪，等待...');
        setTimeout(initApp, 500);
        return;
    }

    try {
        console.log('Tauri API 已就绪，开始初始化');
...
document.addEventListener('change', (e) => {
    if (e.target && e.target.id === 'ipSelect') {
        ...
        invoke('set_selected_ip', { ip: selectedIP }).catch(err => {
